...
//            	'custom3' => 'Custom 3',
			...
			$brand = Verotel\FlexPay\Brand::create_from_merchant_id($this->customer_id);
			$flexpayClient = new Verotel\FlexPay\Client($this->shop_id,$this->sig_key, $brand);
			...
			if ( $this->order_contains_subscription( $order->id ) ) 
			{
				...
				$url = $flexpayClient->get_subscription_URL($params);	
			}
			else
			{
				// Standard purchase
				$url = $flexpayClient->get_purchase_URL($params);
			}
		...
		function check_verotel_response() {
			...
			{	
				if (!$flexpayClient->validate_signature($params))
				{	
		...
		{	
			...
			$url = $flexpayClient->get_status_URL($statusparams);
